...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		// === 로컬스토리지(localStorage)에 저장된 key가 "saveid"인 userid 값을 불러와서 input 태그 userid에 넣어주기 === //
		...
		$("input#loginPwd").keyup(function(event) {
			if (event.keyCode == 13) {// 암호임력란에 엔터를 했을 경우
				goLogin(); // 로그인 시도한다.	
			}
		});
	...
	//	alert("로그인 시도");
		...
		if ( $("input:checkbox[id=saveid]").prop("checked") ) {
			...
			localStorage.setItem('saveid', $("input#loginUserid").val()); // localStorage에 id 저장
		}else {
			//alert("아이디저장 체크를 안 하셨네요");
			
			localStorage.removeItem('saveid'); // localStorage에 id 삭제
		}
	...
	function goCoinPurchaseEnd(userid, coinmoney) {
		//alert("확인용 부모창의 함수 호출함. 사용자 ID : " +userid +", 결제금액 : " + coinmoney + "원");
...
<c:if test="${not empty sessionScope.loginUser}">
 ...
 <%-- ****** 아이디 찾기 Modal ****** --%>
  ...
  <%-- PG(Payment GateWay 결제)에 코인금액을 카드(카카오페이등)로 결제후 DB상에 사용자의 코인액을 update를 해주는 폼이다. --%>
