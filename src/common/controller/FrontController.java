    ...
    */
		...
		try {
		 ...
		 // fis는 C:/NCS/workspace(jsp)/MyMVC/WebContent/WEB-INF/Command.properties 파일의 내용을 읽어오기 위한 용도로 쓰이는 객체이다.
			...
			pr.load(fis);
		/*
			pr.load(fis); 는 fis 객체를 사용하여 C:/NCS/workspace(jsp)/MyMVC/WebContent/WEB-INF/Command.properties 파일의 내용을 읽어다가 
		...
		*/
			...
			*/
			...
			while (en.hasMoreElements()) { // rs.next()와 같이 값이 있다면 순차적으로 가리키며 boolean값을 리턴해준다.
				
				String key = (String)en.nextElement(); // 실제 key 값을 끄집어내 리턴해준다.
				
			//  System.out.println("~~~~ 확인용 key => " + key);
		    //  System.out.println("~~~~ 확인용 value => " + pr.getProperty(key));// pr.getProperty(key) ==> key에 해당하는 value 값을 리턴해준다.
			/*
				~~~~ 확인용 key => /main.up
			...
			*/
				String className = pr.getProperty(key); // key에 해당하는 value값을 꺼냄.
				
				if (className != null) { // 혹시나 공백만이 들어가있는 className이 있다면 ClassNotFoundException로 넘어갈 것이므로 여기에선 NullPointerException만을 거르는 것. 
					
					className = className.trim(); // 혹시나 Command.properties파일에서 value값에 공백이 들어갔다면 제거해주는 것.
					
					Class<?> cls = Class.forName(className);
					// <?>는 generic인데 어떤 클래스 타입인지는 모르지만 어찌됐든 클래스 타입이 들어온다는 뜻.
				...
				*/
					...
					cmdMap.put(key, obj); // 생성된 객체(인스턴스)들을 각자 key값에 매칭시켜 Map에 넣는다.
		...
		} catch (ClassNotFoundException e) { // className값의 클래스가 실제로는 존재하지 않을 수도 있음. 
	...
	private void requestProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	 ...
	 // ~~~ 확인용 uri => /MyMVC/member/idDuplicateCheck.up
	    ...
	    AbstractController action = (AbstractController) cmdMap.get(key); // 다형성을 이용하여 자식 객체가 부모클래스를 받는 것.
	    ...
	    if (action == null) {
			...
			try {
				...
				if (!bool) {
		            ...
		            // forward 되어지면 웹브라우저의 URL주소는 변경되지 않고 그대로 이면서 화면에 보여지는 내용은 forward 되어지는 jsp 파일이다.
