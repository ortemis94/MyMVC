...
public class MallDisplayJSONAction extends AbstractController {
	...
	*/
		...
		String end = String.valueOf(Integer.parseInt(start) + Integer.parseInt(len) - 1); // 연산을 위해 int 타입으로 변경했다가 결과를 다시 String으로 변경.
		...
		if ( prodList.size() > 0 ) {
			for (ProductVO pvo : prodList) {
				...
				// , .....
				// , {......}]
			}// end of for (ProductVO pvo : jsonArr) {})----------------------------------
		...
		//  -- 만약에 select 되어진 정보가 없다면 []로 나오므로 null이 아닌 요소가 없는 빈 배열이다. 
