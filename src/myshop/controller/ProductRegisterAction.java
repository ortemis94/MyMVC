...
public class ProductRegisterAction extends AbstractController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (loginUser != null && loginUser.getUserid().equals("admin")) {
			...
			if (!method.equalsIgnoreCase("POST")) { // GET방식 이라면
				...
				*/
				  ...
				  // !!!!! 크로스 사이트 스크립트 공격에 대응하는 안정한 코드(시큐어코드) 작성하기 !!!!! //
				  String pcontent = mtrequest.getParameter("pcontent");
				  pcontent = pcontent.replaceAll("<", "&lt;"); // 태그(<)를 그냥 부등호(<)로 바꾼다는 뜻.
				  pcontent = pcontent.replaceAll(">", "&gt;"); // 태그(>)를 그냥 부등호(>)로 바꾼다는 뜻.
				  
				  // 입력한 내용에서 엔터는 <br>로 변환시키기
				  pcontent = pcontent.replaceAll("\r\n", "<br>"); // 엔터(\r\n)를 html 줄개행(<br>)으로 바꾼다는 뜻.
				  ...
				  int m = 1;
				  String str_attachCount = mtrequest.getParameter("attachCount"); // 추가 이미지 첨부파일 개수 "0" ~ "10"이 들어온다.
				  
				  int attachCount = 0;
				  
				  if (!str_attachCount.equals("")) { // 추가 이미지 첨부파일 개수 값이 넘어왔다면
					  attachCount = Integer.parseInt(str_attachCount); // 넘어온 값을 넣어준다.
				  }
				  ...
				  }// end of for (int i = 0; i < attachCount; i++) {}--------------------
